...
class TerrainDeformationMethod {
	...
	// PathPoint props: Vector4: innerWidth, outerWidth, splatWidth, centerElevation
	...
	static Vector4 LineSegment(in Vector2 p, in Vector3 a, in Vector3 b, in Vector4 propsA, in Vector4 propsB, ref Vector3 minDist) {
		...
		// Calculate splat.
		float splat = Mathf.Clamp(inverselerp(props.Z + 0.0f, props.Z - 0.5f, dist),0,1);

		// Output weight, influence, height, splat.
		return new Vector4(weight, influence * weight, height * weight, splat);
	}
	...
	public static void ApplySpecs(
		...
		for (uint x = 0; x < GridSizeX; x++) {
			for (uint y = 0; y < GridSizeY; y++) {
				...
				Vector4 splat = new Vector4(0,0,0,0);
				Vector3 dist = dists[(int)index]; //TODO: int might by not enough
				...
				for (int i = 0; i < specCount; i++) {
					...
					if (p.X > bounds.X && p.Y > bounds.Y && p.X < bounds.Z && p.Y < bounds.W) {
						Vector4 currentSplat = specDatas[i].splat;
				...
				if (influence > 0) {
					...
					splats[(int)index] = splats[(int)index].Lerp(splat, Mathf.Clamp(col.W,0,1));
				}
			}
		}
	}
